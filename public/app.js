...
async function boot(){const me=await api('/api/me');if(!me.ok){$('#login').classList.remove('hidden');showAuthStep('#loginStepPassword');return}$('#app').classList.remove('hidden');startAdminIdleGuard();await Promise.all([loadStatus(),loadChats(),loadRules(),loadKb(),loadCanned(),loadPromos(),loadImportant(),loadHuman(),loadBridge(),loadLearning(),loadLogs(),loadCta(),loadHealth(),loadSecurity()]);startLiveUi()}

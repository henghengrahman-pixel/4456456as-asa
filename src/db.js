...
export async function getKnowledge(intent){
  ...
  const r=await brainPool.query(`SELECT category,title,content FROM knowledge_base WHERE active=true AND upper(category)=ANY($1::text[]) ORDER BY CASE WHEN upper(category)='GENERAL' THEN 0 WHEN upper(category)='SYNONYM' THEN 1 ELSE 2 END,id LIMIT 120`,[cats]);
  return r.rows;
}
export async function getRelevantKnowledge(query,intent,limit=20){
  ...
  const r=await brainPool.query(`SELECT id,category,title,content,updated_at FROM knowledge_base WHERE active=true AND upper(category)=ANY($1::text[]) ORDER BY updated_at DESC,id DESC LIMIT 800`,[cats]);
...
export async function captureHumanReplyLearning({chatId,eventId,responseText}){
  ...
  const r=await brainPool.query(`INSERT INTO learning_examples(source_type,intent,member_text,response_text,status,chat_id,source_event_id,context_snapshot,style_only) VALUES('HUMAN_CHAT',$1,$2,$3,'PENDING',$4,$5,$6,false) ON CONFLICT(chat_id,source_event_id) DO NOTHING RETURNING *`,[intent,String(member.text).trim(),String(responseText).trim(),chatId,String(eventId||''),contextSnapshot]);
  return r.rows[0]||null;
}

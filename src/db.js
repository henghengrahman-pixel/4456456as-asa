...
export async function migrate() {
  ...
  ALTER TABLE telegram_bridge_settings ADD COLUMN IF NOT EXISTS token_fingerprint TEXT;
  ...
  CREATE TABLE IF NOT EXISTS runtime_leases (
    lease_key TEXT PRIMARY KEY, holder TEXT NOT NULL, expires_at TIMESTAMPTZ NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
  );
...
export async function getTelegramSettingsInternal(){
  const r=await pool.query(`SELECT bot_token_enc AS "botToken",bot_username AS "botUsername",token_fingerprint AS "tokenFingerprint",default_chat_id AS "defaultChatId",routes,enabled,last_update_id AS "lastUpdateId",updated_at AS "updatedAt" FROM telegram_bridge_settings WHERE id=1`);
  return r.rows[0]||{botToken:'',botUsername:null,tokenFingerprint:null,defaultChatId:'',routes:{},enabled:false,lastUpdateId:0};
}
export async function saveTelegramSettings({botTokenEnc,botUsername=null,tokenFingerprint=null,defaultChatId='',routes={},enabled=null,resetCursor=false}){
  const client=await pool.connect();
  try{
    await client.query('BEGIN');
    const r=await client.query(`UPDATE telegram_bridge_settings SET
      ...
      enabled=COALESCE($5::boolean,enabled),token_fingerprint=COALESCE($6,token_fingerprint),
      last_update_id=CASE WHEN $7::boolean THEN 0 ELSE last_update_id END,updated_at=now() WHERE id=1
      RETURNING bot_token_enc AS "botToken",bot_username AS "botUsername",token_fingerprint AS "tokenFingerprint",default_chat_id AS "defaultChatId",routes,enabled,last_update_id AS "lastUpdateId",updated_at AS "updatedAt"`,
      [botTokenEnc??null,botUsername||null,String(defaultChatId||''),JSON.stringify(routes||{}),enabled==null?null:Boolean(enabled),tokenFingerprint||null,Boolean(resetCursor)]);
    if(resetCursor) await client.query(`DELETE FROM telegram_processed_updates`);
    await client.query('COMMIT');
    return r.rows[0];
  }catch(e){ await client.query('ROLLBACK').catch(()=>{}); throw e; } finally{ client.release(); }
}
export async function claimRuntimeLease(leaseKey,holder,ttlSeconds=10){
  const ttl=Math.max(3,Math.min(Number(ttlSeconds)||10,120));
  const r=await pool.query(`INSERT INTO runtime_leases(lease_key,holder,expires_at,updated_at) VALUES($1,$2,now()+($3::text||' seconds')::interval,now())
    ON CONFLICT(lease_key) DO UPDATE SET holder=EXCLUDED.holder,expires_at=EXCLUDED.expires_at,updated_at=now()
    WHERE runtime_leases.expires_at < now() OR runtime_leases.holder=EXCLUDED.holder
    RETURNING holder`,[String(leaseKey),String(holder),String(ttl)]);
  return r.rowCount>0 && String(r.rows[0]?.holder||'')===String(holder);
}

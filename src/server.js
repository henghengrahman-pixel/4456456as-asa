...
async function syncTelegramFromEnv(){
  ...
  let enc=null, username=current.botUsername||null, tokenFingerprint=current.tokenFingerprint||null, resetCursor=false;
  if(token){
    ...
    let previousToken=''; try{ previousToken=current.botToken?decryptSecret(current.botToken):''; }catch{}
    resetCursor=!previousToken || previousToken!==token || (current.tokenFingerprint && current.tokenFingerprint!==tokenFingerprint);
...
app.get('/health', async(req,res)=>{
  ...
  const ok=db&&brainDb;
  res.status(ok?200:503).json({ok,service:'livechat-ai',db,brainDb,sharedBrain:Boolean(config.brainDatabaseUrl&&config.brainDatabaseUrl!==config.databaseUrl),livechatConfigured:lc.ready(),openaiConfigured:ai.ready(),poller:pollerStatus(),integrations,warnings:validateConfig()});
});
...
app.post('/api/login/2fa/verify',async(req,res)=>{
  try{
    ...
    if(!verified) return res.status(401).json({ok:false,error:'TWOFA_INVALID'});
...
app.put('/api/human-bridge/settings',requireAdmin,async(req,res)=>{
  try{
    ...
    let enc=null, username=null, tokenFingerprint=null, resetCursor=false;
    if(token){
      const current=await getTelegramSettingsInternal();
      const tg=new TelegramClient(token); const me=await tg.getMe();
      await tg.deleteWebhook();
      enc=encryptSecret(token); username=me.username||null; tokenFingerprint=crypto.createHash('sha256').update(token).digest('hex');
      let previousToken=''; try{ previousToken=current.botToken?decryptSecret(current.botToken):''; }catch{}
      resetCursor=!previousToken || previousToken!==token || (current.tokenFingerprint && current.tokenFingerprint!==tokenFingerprint);
    }
    const saved=await saveTelegramSettings({botTokenEnc:enc,botUsername:username,tokenFingerprint,defaultChatId,routes,enabled:null,resetCursor});
